...
def dashboard():
    ...
    try:
        ...
        db_conn = client[os.getenv('DB_NAME', 'sih2573')]
...
def user_dashboard():
    ...
    try:
        ...
        profile = {
            ...
            'profile_pic': user.get('photo', user.get('profile_pic', None)),
            '_id': user['_id'],
...
configured_secret_key = os.getenv('SECRET_KEY') or os.getenv('FLASK_SECRET_KEY')
if configured_secret_key:
    app.secret_key = configured_secret_key
else:
    # Keep a deterministic fallback so sessions remain valid across workers.
    app.secret_key = 'deepfit-session-secret-change-me'
    log_warning("SECRET_KEY not set; using built-in fallback key. Set SECRET_KEY in production.")
...
# Helper to obtain a MongoDB database connection
def get_db():
...
def normalize_email(email):
    """Normalize email for reliable cross-collection lookups."""
    if not email:
        return None
...
def get_session_user_email():
    """
    Resolve current user email from session with DB fallback.
    Keeps session email normalized for consistent downstream queries.
    """
    resolved_email = normalize_email(session.get('user_email'))
    user_id = session.get('user_id')

    if not user_id or not mongodb_connected:
        return resolved_email

    try:
        user_doc = db.users.find_one({'_id': ObjectId(user_id)})
        if user_doc and user_doc.get('email'):
            db_email = normalize_email(user_doc.get('email'))
            if db_email:
                resolved_email = db_email
                session['user_email'] = db_email
    except Exception as lookup_error:
        print(f"Session email lookup warning: {lookup_error}")

    return resolved_email

def build_email_query(email, include_legacy_email=False):
    """Create a case-insensitive Mongo query for user email fields."""
...
def build_user_identity_query(user_email=None, user_id=None, include_legacy_email=False):
    """Build a resilient query using email and legacy user_id fields."""
    query_parts = []

    email_query = build_email_query(user_email, include_legacy_email=include_legacy_email)
    if email_query:
        email_parts = email_query.get('$or')
        if isinstance(email_parts, list):
            query_parts.extend(email_parts)
        else:
            query_parts.append(email_query)

    if user_id:
        user_id_str = str(user_id)
        query_parts.append({'user_id': user_id_str})
        try:
            query_parts.append({'user_id': ObjectId(user_id_str)})
        except Exception:
            pass
...
def build_user_id_query(user_id):
    """Build query clauses for user_id stored as string or ObjectId."""
    if not user_id:
        return None

    user_id_str = str(user_id)
    conditions = [{'user_id': user_id_str}]
    try:
        conditions.append({'user_id': ObjectId(user_id_str)})
    except Exception:
        pass
...
def parse_dashboard_datetime(value):
    """Parse mixed datetime formats used across modern + legacy records."""
    if not value:
        return None
...
def sort_dashboard_docs(docs, timestamp_fields):
    """Sort records by first parseable timestamp from candidate fields."""
    def extract_sort_value(doc):
        for field in timestamp_fields:
            parsed = parse_dashboard_datetime(doc.get(field))
            if parsed:
                return parsed
        return datetime.min

    return sorted(docs, key=extract_sort_value, reverse=True)

def get_legacy_exercise_records(exercise_key, user_id, limit=20):
    """
    Pull legacy exercise data from exercise_sessions/exercise_results and map to
    the current dashboard schema for each exercise.
    """
    if not mongodb_connected or not user_id:
        return []

    user_query = build_user_id_query(user_id)
    if not user_query:
        return []

    type_patterns = {
        'situp': r'sit[\s_-]*ups?|crunch',
        'dumbbell': r'dumbbell|curl|bicep',
        'jump': r'vertical[\s_-]*jump|jump'
    }
    type_pattern = type_patterns.get(exercise_key)
    if not type_pattern:
        return []
    ...
    try:
        sessions_query = {
            '$and': [
                user_query,
                {'exercise_type': {'$regex': type_pattern, '$options': 'i'}}
            ]
        }
        legacy_sessions = list(
            db.exercise_sessions.find(sessions_query).sort('date', -1).limit(max(limit * 3, 20))
        )
    except Exception as lookup_error:
        log_error(f"Legacy exercise_sessions lookup error ({exercise_key}): {lookup_error}")
        legacy_sessions = []

    result_by_session = {}
    try:
        session_ids = [s.get('_id') for s in legacy_sessions if s.get('_id') is not None]
        if session_ids:
            legacy_results = list(db.exercise_results.find({'session_id': {'$in': session_ids}}))
            for result in legacy_results:
                session_id = result.get('session_id')
                if not session_id:
                    continue
                existing = result_by_session.get(session_id)
                if not existing:
                    result_by_session[session_id] = result
                    continue
                existing_ts = parse_dashboard_datetime(existing.get('analyzed_at')) or datetime.min
                current_ts = parse_dashboard_datetime(result.get('analyzed_at')) or datetime.min
                if current_ts > existing_ts:
                    result_by_session[session_id] = result
    except Exception as join_error:
        log_error(f"Legacy exercise_results join error ({exercise_key}): {join_error}")

    normalized = []
    for session_doc in legacy_sessions:
        reps_raw = session_doc.get('repetitions')
        try:
            reps_value = float(reps_raw)
        except (TypeError, ValueError):
            reps_value = None
        if reps_value is None or reps_value <= 0:
            continue

        reps_number = int(reps_value) if reps_value.is_integer() else reps_value
        session_id = session_doc.get('_id')
        result_doc = result_by_session.get(session_id, {})
        timestamp = (
            session_doc.get('date') or
            session_doc.get('created_at') or
            result_doc.get('analyzed_at')
        )

        if exercise_key == 'situp':
            normalized.append({
                'reps_completed': reps_number,
                'form_quality': result_doc.get('form_score'),
                'submission_time': timestamp,
                'created_at': timestamp,
                'legacy_source': 'exercise_sessions'
            })
        elif exercise_key == 'dumbbell':
            normalized.append({
                'total_reps': reps_number,
                'analysis_date': timestamp.isoformat() if isinstance(timestamp, datetime) else timestamp,
                'submission_time': timestamp,
                'timestamp': timestamp,
                'exercise_type': session_doc.get('exercise_type') or 'dumbbell_curls',
                'legacy_source': 'exercise_sessions'
            })
        elif exercise_key == 'jump':
            normalized.append({
                'total_jumps': reps_number,
                'session_start': timestamp.isoformat() if isinstance(timestamp, datetime) else timestamp,
                'submission_time': timestamp,
                'legacy_source': 'exercise_sessions'
            })

    # Optional fallback: some old deployments may have exercise_results without session linkage.
    try:
        fallback_results_query = {
            '$and': [
                user_query,
                {'exercise_type': {'$regex': type_pattern, '$options': 'i'}},
                {'repetitions': {'$gt': 0}}
        ...
        fallback_results = list(
            db.exercise_results.find(fallback_results_query).sort('analyzed_at', -1).limit(limit)
        )
        for result_doc in fallback_results:
            reps_raw = result_doc.get('repetitions')
            try:
                reps_value = float(reps_raw)
            except (TypeError, ValueError):
                continue
            if reps_value <= 0:
                continue
            reps_number = int(reps_value) if reps_value.is_integer() else reps_value
            timestamp = result_doc.get('analyzed_at') or result_doc.get('date')
            ...
            if exercise_key == 'situp':
                normalized.append({
                    ...
                    'submission_time': timestamp,
                    'created_at': timestamp,
                    'legacy_source': 'exercise_results'
                })
            elif exercise_key == 'dumbbell':
                normalized.append({
                    'total_reps': reps_number,
                    ...
                    'submission_time': timestamp,
                    'timestamp': timestamp,
                    'exercise_type': result_doc.get('exercise_type') or 'dumbbell_curls',
                    'legacy_source': 'exercise_results'
                })
            elif exercise_key == 'jump':
                normalized.append({
        ...
        # Fallback query intentionally best-effort.
        pass
...
def get_latest_measurement():
    ...
    try:
        ...
        # Resolve user email robustly: prefer DB user, fallback to session email.
        user_email = None
        session_email = session.get('user_email')
        if session_email:
            user_email = str(session_email).strip().lower()
        ...
        if user_id:
            try:
                ...
                print(f"User lookup fallback used due to error: {lookup_error}")

        if not user_email:
            print("No user email resolved from session")
            return jsonify({'success': False, 'error': 'User not found'}), 404

        print(f"Looking for measurements for user: {user_email}")
        
        # Try to get final estimate first (most recent) - use flexible email query
        final_estimate = None
        try:
            email_query = build_email_query(user_email, include_legacy_email=True)
            if email_query:
        ...
        if final_estimate:
            ...
            # Return final estimate data with proper field mapping
            return jsonify({
                'success': True,
                'data': {
                    ...
                    'confidence_score': float(str(final_estimate.get('confidence_level', '80%')).replace('%', '')) / 100 if final_estimate.get('confidence_level') else 0.8,
        ...
        # Fallback to latest individual measurement - use flexible email query
...
def debug_data():
    ...
    try:
        debug_info = {
            ...
            'height_weight': [],
            'exercise_data': {},
            'legacy_exercise_data': {},
            'user_session': session.get('user_email', 'No session'),
            'user_id': session.get('user_id', 'No user_id')
        }
        ...
        if session and 'user_id' in session:
            ...
            if user:
                ...
                debug_info['resolved_email'] = user_email
        
        # Check final estimates with flexible query
        try:
            if user_email:
                email_query = build_email_query(user_email, include_legacy_email=True)
                final_estimates = list(db['Final_Estimated_Height_and_Weight'].find(email_query).sort('timestamp', -1).limit(5))
            else:
            ...
            for est in final_estimates:
                est['_id'] = str(est['_id'])
                if 'timestamp' in est:
                    est['timestamp'] = str(est['timestamp'])
            debug_info['final_estimates'] = final_estimates
            debug_info['final_estimates_count'] = len(final_estimates)
        except Exception as e:
            debug_info['final_estimates_error'] = str(e)
        
        # Check height weight measurements with flexible query
        try:
            if user_email:
                email_query = build_email_query(user_email, include_legacy_email=True)
                height_weight = list(db['Height and Weight'].find(email_query).sort('timestamp', -1).limit(5))
            else:
            ...
            for hw in height_weight:
                hw['_id'] = str(hw['_id'])
                if 'timestamp' in hw:
                    hw['timestamp'] = str(hw['timestamp'])
            debug_info['height_weight'] = height_weight
            debug_info['height_weight_count'] = len(height_weight)
        except Exception as e:
        ...
        if user_email:
            ...
            try:
                email_query = build_email_query(user_email, include_legacy_email=True)
                situps = list(db.situps.find(email_query).sort('submission_time', -1).limit(3))
                for s in situps:
                    s['_id'] = str(s['_id'])
                    if 'submission_time' in s:
                        s['submission_time'] = str(s['submission_time'])
                debug_info['exercise_data']['situps'] = situps
                debug_info['exercise_data']['situps_count'] = len(situps)
            except Exception as e:
            ...
            try:
                email_query = build_email_query(user_email, include_legacy_email=True)
                dumbbell = list(db.DumbBell.find(email_query).sort('submission_time', -1).limit(3))
                for d in dumbbell:
                    d['_id'] = str(d['_id'])
                    if 'submission_time' in d:
                        d['submission_time'] = str(d['submission_time'])
                debug_info['exercise_data']['dumbbell'] = dumbbell
                debug_info['exercise_data']['dumbbell_count'] = len(dumbbell)
            except Exception as e:
            ...
            try:
                email_query = build_email_query(user_email, include_legacy_email=True)
                jumps = list(db.Vertical_Jump.find(email_query).sort('session_start', -1).limit(3))
                for j in jumps:
                    j['_id'] = str(j['_id'])
                    if 'session_start' in j:
                        j['session_start'] = str(j['session_start'])
                debug_info['exercise_data']['vertical_jump'] = jumps
                debug_info['exercise_data']['vertical_jump_count'] = len(jumps)
            except Exception as e:
                debug_info['exercise_data']['vertical_jump_error'] = str(e)

            # Check legacy exercise sessions/results
            try:
                user_id_query = build_user_id_query(user_id)
                if user_id_query:
                    legacy_sessions = list(db.exercise_sessions.find(user_id_query).sort('date', -1).limit(5))
                    ...
                    session_ids = [ObjectId(s['_id']) for s in legacy_sessions if s.get('_id')]
                    legacy_results = list(db.exercise_results.find({'session_id': {'$in': session_ids}}).sort('analyzed_at', -1).limit(5)) if session_ids else []
                    for r in legacy_results:
                        r['_id'] = str(r['_id'])
                        if isinstance(r.get('session_id'), ObjectId):
                    ...
                    debug_info['legacy_exercise_data']['sessions'] = legacy_sessions
                    debug_info['legacy_exercise_data']['sessions_count'] = len(legacy_sessions)
                    debug_info['legacy_exercise_data']['results'] = legacy_results
                    debug_info['legacy_exercise_data']['results_count'] = len(legacy_results)
                else:
                    debug_info['legacy_exercise_data']['sessions_count'] = 0
                    debug_info['legacy_exercise_data']['results_count'] = 0
            except Exception as e:
                debug_info['legacy_exercise_data']['error'] = str(e)
...
def submit_situp_result():
    ...
    try:
        ...
        user_email = get_session_user_email()
        if not user_email:
            return jsonify({'success': False, 'error': 'Not authenticated'}), 401
        ...
        if mongodb_connected:
            ...
            print(f"✅ MongoDB Save SUCCESS! ID: {result_id}")
...
def get_latest_situp_result():
    ...
    try:
        user_id = session.get('user_id')
        user_email = get_session_user_email()
        identity_query = build_user_identity_query(
            user_email=user_email,
            user_id=user_id,
            include_legacy_email=True
        )
...
def get_situp_data():
    ...
    try:
        if mongodb_connected:
            ...
            situps_collection = db['situps']
            modern_data = list(situps_collection.find(identity_query).sort('submission_time', -1).limit(40))
            legacy_data = get_legacy_exercise_records('situp', user_id, limit=40)
            data = modern_data + legacy_data
            data = sort_dashboard_docs(data, ['submission_time', 'created_at', 'timestamp'])[:20]
...
def get_dumbbell_data():
    ...
    try:
        if mongodb_connected:
            ...
            dumbbell_collection = db['DumbBell']
            modern_data = list(dumbbell_collection.find(identity_query).sort('analysis_date', -1).limit(40))
            legacy_data = get_legacy_exercise_records('dumbbell', user_id, limit=40)
            data = modern_data + legacy_data
            data = sort_dashboard_docs(data, ['analysis_date', 'submission_time', 'timestamp'])[:20]
...
def submit_vertical_jump_result():
    ...
    try:
        ...
        print(f"\n🔥 API: SAVING VERTICAL JUMP RESULT 🔥")
        print(f"✅ User Email: {user_email}")
        print(f"📊 Data: {result_data}")
...
def submit_dumbbell_result():
    ...
    try:
        ...
        if mongodb_connected:
            ...
            print(f"✅ MongoDB Save SUCCESS! ID: {result_id}")
            print(f"📁 Saved to: Database=sih2573, Collection=DumbBell")
...
def get_vertical_jump_data():
    ...
    try:
        if mongodb_connected:
            ...
            if not identity_query:
                # Return sample data if no user email
                sample_data = create_sample_vertical_jump_data()
                return jsonify(sample_data)
            
            # Use the correct collection name from MongoDB
            jump_collection = db['Vertical_Jump']
            modern_data = list(jump_collection.find(identity_query).sort('session_start', -1).limit(40))
            legacy_data = get_legacy_exercise_records('jump', user_id, limit=40)
            data = modern_data + legacy_data
            data = sort_dashboard_docs(data, ['session_start', 'submission_time', 'timestamp'])[:20]
...
def get_dynamic_benchmarks():
    try:
        email = normalize_email(request.args.get('email')) or get_session_user_email()
        if not email:
