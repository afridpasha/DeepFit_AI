...
class AdvancedJumpDetector:
    ...
    def draw_advanced_ui(self, frame):
        ...
        panel_x = 10
        panel_y = 10
        panel_width = max(140, int(w * 0.22))
        panel_height = max(88, int(h * 0.24))
        panel_right = panel_x + panel_width
        panel_bottom = panel_y + panel_height

        cv2.rectangle(frame, (panel_x, panel_y), (panel_right, panel_bottom), (0, 0, 0), -1)
        cv2.rectangle(frame, (panel_x, panel_y), (panel_right, panel_bottom), (255, 255, 255), 1)
        ...
        text_x = panel_x + 6
        text_y_start = panel_y + 16
        line_gap = max(14, int(panel_height * 0.17))
        font_scale = 0.30
        ...
        for i, stat in enumerate(stats):
            cv2.putText(
                frame,
                stat,
                (text_x, text_y_start + i * line_gap),
    ...
    def save_to_mongodb(self):
        ...
        try:
            ...
            session_data = {
                ...
                'calibration_data': {
                    'pixels_per_cm': self.pixels_per_cm,
                    'baseline_y': self.baseline_y if hasattr(self, 'baseline_y') else 0
                },
