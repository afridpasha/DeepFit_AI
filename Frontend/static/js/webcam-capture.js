...
async function initWebcam() {
    try {
        ...
        localStream = await navigator.mediaDevices.getUserMedia({
            video: {
                // Use higher-resolution frames so UI overlays and background stay clear.
...
function startFrameStreaming(eventName = 'video_frame') {
    ...
    frameInterval = setInterval(() => {
        ...
        // Keep compression moderate to preserve overlay details.
        const imageData = canvasElement.toDataURL('image/jpeg', 0.82);
...
socket.on('processed_height_weight_frame', (data) => {
    ...
    // Forward height/weight frame payload so page-specific UI can stay in sync
    // without opening a second Socket.IO connection.
    if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('height_weight_frame_processed', { detail: data }));
    }
});
